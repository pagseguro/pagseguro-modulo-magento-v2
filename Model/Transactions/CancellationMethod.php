...
class CancellationMethod
{
        ...
        if ($this->_PagSeguroPaymentList->getTransactions()) {
            foreach ($this->_PagSeguroPaymentList->getTransactions() as $payment) {
                ...
                if ($this->getStoreReference() == \UOL\PagSeguro\Helper\Data::getReferenceDecrypt($payment->getReference())) {
                    ...
                    if ($order) {
                        if ($order->getStatus() == \UOL\PagSeguro\Helper\Data::getStatusFromKey(1)
                            || $order->getStatus() == \UOL\PagSeguro\Helper\Data::getStatusFromKey(2)) {
                            array_push($this->_arrayPayments, $this->build($payment, $order));
                        }
                    }
                }
    ...
    {
        if ($order->getStatus() == \UOL\PagSeguro\Helper\Data::getStatusFromKey($payment->getStatus()))
            return true;
        return false;
    }
