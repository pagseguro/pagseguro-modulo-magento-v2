...
function validateCreditCard(self) {
  if (
    self.validity.valid &&
    removeNumbers(unmask(self.value)) === '' &&
    self.value.length >= 14 &&
    self.value.length <= 22
  ) {
...
function validateCardHolder(self) {
  if (
    self.validity.tooShort ||
    !self.validity.valid ||
    removeLetters(unmask(self.value)) !== ''
  ) {
    displayError(self);
    return false;
  } else {
...
function cardInstallment(data) {
  ...
  data.forEach(function (item) {
    select.options[select.options.length] = new Option(
      item.quantity + 'x de R$ ' + item.installmentAmount,
      JSON.stringify(item)
    );
  });
  if (data) {
...
function getBrand(self) {
  ...
  PagSeguroDirectPayment.setSessionId(
    document.getElementById('pagseguroCcSessionId').value
  );
  if (validateCreditCard(self)) {
    PagSeguroDirectPayment.getBrand({
      cardBin: unmask(
        document.getElementById('pagseguro_credit_card_number').value
      ),
      success: function (response) {
        document.getElementById('creditCardBrand').value = response.brand.name;
        getInstallments(response.brand.name);
        displayError(
          document.getElementById('pagseguro_credit_card_number'),
          false
        );
      },
      error: function () {
...
function validateCreditCardForm() {
  if (
    validateCreditCard(
      document.querySelector('#pagseguro_credit_card_number')
    ) &&
    ...
    validateCreditCardHolderBirthdate(
      document.querySelector('#creditCardHolderBirthdate')
    ) &&
    validateCreditCardMonth(
      document.querySelector('#creditCardExpirationMonth')
    ) &&
    validateCreditCardYear(
      document.querySelector('#creditCardExpirationYear')
    ) &&
    validateCreditCardCode(document.querySelector('#creditCardCode')) &&
    validateCreditCardInstallment(
      document.querySelector('#card_installment_option')
    )
  ) {
    return true;
  }
