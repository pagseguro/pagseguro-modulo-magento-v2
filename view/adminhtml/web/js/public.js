        ...
        jQuery
          .ajax({
            ...
            data: {
              ...
              days: jQuery('#conciliation-days').val(),
            },
            type: 'POST',
            showLoader: true,
          })
            ...
            if (response.success) {
              ...
              if (response.payload.data.length > 0) {
                ...
                response.payload.data.forEach(function (item) {
                  t.row.add([
                    "<input type='checkbox' data-target='conciliation' data-block='" +
                      item.details +
                      "'/>",
                    item.date,
                    item.magento_id,
                ...
                Modal.Load(
                  'Conciliação',
                  'Sem resultados para o período solicitado.'
                );
              }
            } else {
              //Alert
              Modal.Load(
                'Conciliação',
                'Não foi possível executar esta ação. Tente novamente mais tarde.'
              );
            }
          });
      },
          ...
          .success(function (response) {
            if (response.success) {
              if (response.success === true) {
                ...
                Modal.Load(
                  'Conciliação',
                  'Transações conciliadas com sucesso!'
                );
              }
              ...
              if (response.success === false) {
                ...
                Modal.Load(
                  'Conciliação',
                  'Não foi possível executar esta ação. Utilize a conciliação de transações primeiro ou tente novamente mais tarde.'
                );
              }
            } else {
              //Alert
      ...
      Search: function (url) {
        jQuery
          .ajax({
            ...
            data: {
              form_key: window.FORM_KEY,
              days: jQuery('#abandoned-days').val(),
            },
            type: 'POST',
            showLoader: true,
          })
          .success(function (response) {
            if (response.success) {
              ...
              if (response.payload.data.length > 0) {
                ...
                response.payload.data.forEach(function (item) {
                  t.row.add([
                    "<input type='checkbox' data-target='abandoned' data-block='" +
                      item.details +
                      "'/>",
                    item.date,
                    item.magento_id,
          ...
          .success(function (response) {
            if (response.success) {
              ...
              if (response.success === false) {
                ...
                Modal.Load(
                  'Abandonadas',
                  'Não foi possível executar esta ação. Utilize a recuperação de transações primeiro ou tente novamente mais tarde.'
                );
              }
            } else {
              //Alert
      ...
      Search: function (url) {
        jQuery
          .ajax({
            ...
            data: {
              form_key: window.FORM_KEY,
              days: jQuery('#cancellation-days').val(),
            },
            type: 'POST',
            showLoader: true,
          })
              ...
              if (response.payload.data.length > 0) {
                ...
                response.payload.data.forEach(function (item) {
                  t.row.add([
                    ...
                    item.pagseguro_id,
                    item.magento_status,
                    '<a class="cancellation-cancel" data-target="cancellation_' +
                      i +
                      '" data-block="' +
                      item.details +
                      '">Cancelar</a>',
              ...
              if (response.payload.data.length > 0) {
                ...
                response.payload.data.forEach(function (item) {
                  t.row.add([
                    ...
                    item.pagseguro_id,
                    item.magento_status,
                    '<a class="refund" data-target="refund_' +
                      i +
                      '" data-block="' +
                      item.details +
                      '" data-id="' +
                      item.magento_id +
                      '" style="cursor:pointer;">Estorno total</a><br/>' +
                      '<a class="partial-refund" data-target="refund_' +
                      i +
                      '" data-block="' +
                      item.details +
                      '" data-value="' +
                      ...
                      '" data-id="' +
                      item.magento_id +
                      '" style="cursor:pointer;">Estorno parcial</a>',
                  ]);
                  //Adjust column width
                  t.columns.adjust().draw(false);
                  i++;
          ...
          .success(function (response) {
            if (response.success) {
              ...
              if (response.payload.error === 'Need to conciliate') {
                ...
                response.payload.error === '14002' ||
                response.payload.error === '14013'
              ) {
                Modal.Load(
                  'Estorno',
                  'Valor do estorno está em um formato inválido!'
                );
              } else if (response.payload.error === '14003') {
                Modal.Load(
                  'Estorno',
                  'Valor do estorno inválido! O valor não pode ser negativo.'
                );
              } else if (response.payload.error === '14004') {
                Modal.Load(
                  'Estorno',
                  'Valor do estorno é menor do que o permitido.'
                );
              } else if (response.payload.error === '14005') {
                Modal.Load(
                  'Estorno',
                  'Valor do estorno é maior do que o permitido.'
                );
                ...
                Modal.Load(
                  'Estorno',
                  'Status da transação é inválido para ser estornada.'
                );
                ...
                Modal.Load(
                  'Estorno',
                  "Sua conta PagSeguro não tem permissão para realizar esta ação. Em caso de dúvidas acesse <a href='http://forum.pagseguro.uol.com.br' target='_blank'>http://forum.pagseguro.uol.com.br</a>"
                );
              } else {
                Modal.Load(
                  'Estorno',
              ...
              if (response.payload.data.length > 0) {
                ...
                response.payload.data.forEach(function (item) {
                  t.row.add([
                    ...
                    item.environment,
                    item.magento_status,
                    '<a class="link" data-transaction="' +
                      item.pagseguro_id +
                      '" data-order="' +
                      item.order_id +
                      '">Ver detalhes do pagamento</a>',
                  ]);
          ...
          .success(function (result) {
            if (result.success) {
              ...
              if (
                ...
                listTransactionLine1.append(
                  '<dl><dt>Código de referência: </dt><dd>' +
                    result.reference +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listTransactionLine2.append(
                  '<dl><dt>Origem do cancelamento: </dt><dd>' +
                    result.cancelationSource +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listTransactionLine2.append(
                  '<dl><dt>Total de itens: </dt><dd>' +
                    result.itemCount +
                    '</dd></dl>'
                );
              }

              jQuery('#transaction-group').append(
                '<span id="btn-hidden-itens" class="link ">Exibir todos os itens &#9660</span>'
              );

              jQuery('#transaction-group').append(
                '<div id="itens" class="hidden-groups table"></div>'
              );
              ...
              if (
                ...
                listPaymentLine1.append(
                  '<dl><dt>Tipo de pagamento: </dt><dd>' +
                    result.paymentMethod.titleType +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine1.append(
                  '<dl><dt>Meio de pagamento: </dt><dd>' +
                    result.paymentMethod.titleCode +
                    '</dd></dl>'
                );
              }
              ...
              if (result.paymentLink !== false) {
                listPaymentLine1.append(
                  '<dl><dt>Link para pagamento: </dt><dd>' +
                    '<a href=' +
                    result.paymentLink +
                    '>' +
              ...
              if (
                ...
                listPaymentLine2.append(
                  '<dl class=""><dt>Valor extra: </dt><dd>R$ ' +
                    formatReal(result.extraAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine2.append(
                  '<dl class=""><dt>Desconto: </dt><dd> R$' +
                    formatReal(result.discountAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine2.append(
                  '<dl class=""><dt>Valor bruto: </dt><dd>R$ ' +
                    formatReal(result.grossAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine2.append(
                  '<dl class=""><dt>Valor líquido: </dt><dd>R$ ' +
                    formatReal(result.netAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine2.append(
                  '<dl><dt>Código de promoção: </dt><dd>' +
                    result.promoCode +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listPaymentLine1.append(
                  '<dl><dt>Data de crédito: </dt><dd>' +
                    result.escrowEndDate +
                    '</dd></dl>'
                );
              }

              jQuery('#payment-group').append(
                '<span id="btn-hidden-data" class="link">Exibir todos os dados de custos cobrados &#9660</span>'
              );

              jQuery('#payment-group').append(
                '<div id="cost-data" class="hidden-groups table"></div>'
              );
              ...
              if (result.itemCount > 0) {
                ...
                listItensTitle.append(
                  '<div class="description-cell">Produto</div>'
                );
                listItensTitle.append(
                  '<div class="itens-cell">Quantidade</div>'
                );
                ...
                for (var i = 0; i < result.itemCount; i++) {
                  var total = (
                    result.items[i].quantity * result.items[i].amount
                  ).toFixed(2);
                  listItensBody = jQuery('.group-table-itens');
                  listItensBody.append(
                    '<div id="item' + i + '" class="itens-line"></div>'
                  );
                  listItensBody = jQuery('#item' + i);
                  listItensBody.append(
                    '<div class="itens-cell">' + result.items[i].id + '</div>'
                  );
                  listItensBody.append(
                  ...
                  listItensBody.append(
                    '<div class="itens-cell">R$ ' +
                      formatReal(result.items[i].amount) +
                      '</div>'
                  );
                  listItensBody.append(
                    '<div class="itens-cell"> R$ ' +
              ...
              if (
                ...
                listData.append(
                  '<dl><dt>Taxa de parcelamento: </dt><dd> R$' +
                    formatReal(result.creditorFees.installmentFeeAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listData.append(
                  '<dl><dt>Taxa de operação: </dt><dd> R$' +
                    formatReal(result.creditorFees.operationalFeeAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listData.append(
                  '<dl><dt>Tarifa de intermediação: </dt><dd>R$ ' +
                    formatReal(result.creditorFees.intermediationRateAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listData.append(
                  '<dl><dt>Taxa de intermediação: </dt><dd>R$ ' +
                    formatReal(result.creditorFees.intermediationFeeAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (
                ...
                listData.append(
                  '<dl><dt>Taxa de comissão: </dt><dd>R$ ' +
                    formatReal(result.creditorFees.comissionFeeAmount) +
                    '</dd></dl>'
                );
              }
              ...
              if (result.payload.error === 'need to conciliate') {
                Modal.Load(
                  'Atenção',
                  'É necessário utilizar a conciliação de transações primeiro.'
                );
              }
            }
          });
...
function validateSearchByDate() {
  ...
  if (
    (fieldDateFromValue.length > 0 && fieldDateToValue.length === 0) ||
    (fieldDateFromValue.length === 0 && fieldDateToValue.length > 0)
  ) {
    Modal.Load(
      'Erro ao filtrar!',
      'Data de início e fim devem ser informadas!'
    );
    return false;
  }
